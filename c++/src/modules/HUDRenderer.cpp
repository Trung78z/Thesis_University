...
void HUDRenderer::render(cv::Mat &image, float ego_speed, int acc_speed,
                         ...
                         int target_id, float engine_force,
                         float throttle_force, float brake_force) {
    ...
    cv::putText(image,
                ...
                cv::Point(120, 100), font, font_scale, cv::Scalar(255, 20, 20),
                thickness);
    ...
    cv::putText(image, "Engine: " + std::to_string((int)engine_force) + " N",
                cv::Point(200, 125), font, font_scale, cv::Scalar(255, 200, 0),
                thickness);
    cv::putText(image,
                "Throttle: " + std::to_string((int)throttle_force) + " N",
                cv::Point(200, 145), font, font_scale, cv::Scalar(0, 255, 255),
                thickness);
    cv::putText(image, "Brake: " + std::to_string((int)brake_force) + " N",
                cv::Point(200, 165), font, font_scale, cv::Scalar(0, 128, 255),
                thickness);
